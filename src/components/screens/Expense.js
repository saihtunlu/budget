...
import {getExpense, getTotalExpenseByDate} from '../../core/redux/getters';
...
function Expense(props) {
  const {Expenses, TotalExpense, Theme, navigation} = props;
  ...
  const getData = (date_) => {
    ...
    setExpenses(Expenses(date_));
    setTotalExpense(TotalExpense(date_));
  };
  const editItem = (item, index) => {
    item.price = item.price.toString();
    navigation.push('EditExpense', {
      expense: item,
      index: index,
    });
  };

  const prevDateExpense = () => {
    const lastDay = moment(date).subtract(1, 'days');
    setDate(lastDay);
    getData(lastDay);
  };

  const nextDateExpense = () => {
...
const mapStateToProps = (state) => ({
  ...
  Expenses: (date) => getExpense(state.expense, date),
  TotalExpense: (date) => getTotalExpenseByDate(state.expense, date),
});
